 ...
 * </copyright>
 */
package com.ahaxt.hanlp.mining.word2vec;

import com.ahaxt.hanlp.algorithm.MaxHeap;
...
{
    Map<K, com.ahaxt.hanlp.mining.word2vec.Vector> storage;

    public AbstractVectorModel(Map<K, com.ahaxt.hanlp.mining.word2vec.Vector> storage)
    {
    ...
    {
        storage = new TreeMap<K, com.ahaxt.hanlp.mining.word2vec.Vector>();
    }
     ...
     * @return 向量
     */
    final public com.ahaxt.hanlp.mining.word2vec.Vector vector(K key)
    {
        com.ahaxt.hanlp.mining.word2vec.Vector vector = storage.get(key);
    ...
    {
        com.ahaxt.hanlp.mining.word2vec.Vector vectorWhat = storage.get(what);
        if (vectorWhat == null)
        {
            return -1f;
        }
        com.ahaxt.hanlp.mining.word2vec.Vector vectorWith = storage.get(with);
        if (vectorWith == null)
        {
            return -1f;
        }
     ...
     * @return 键值对列表, 键是相似词语, 值是相似度, 按相似度降序排列
     */
    private List<Map.Entry<K, Float>> nearest(K key, com.ahaxt.hanlp.mining.word2vec.Vector vector, int size)
    {
        ...
        for (Map.Entry<K, com.ahaxt.hanlp.mining.word2vec.Vector> entry : storage.entrySet())
        {
     ...
     * @return 键值对列表, 键是相似词语, 值是相似度, 按相似度降序排列
     */
    public List<Map.Entry<K, Float>> nearest(com.ahaxt.hanlp.mining.word2vec.Vector vector, int size)
    {
        MaxHeap<Map.Entry<K, Float>> maxHeap = new MaxHeap<Map.Entry<K, Float>>(size, new Comparator<Map.Entry<K, Float>>()
        {
            @Override
     ...
     * @return 键值对列表, 键是相似词语, 值是相似度, 按相似度降序排列
     */
    public List<Map.Entry<K, Float>> nearest(com.ahaxt.hanlp.mining.word2vec.Vector vector)
    {
     ...
     * @return
     */
    public abstract com.ahaxt.hanlp.mining.word2vec.Vector query(String query);
