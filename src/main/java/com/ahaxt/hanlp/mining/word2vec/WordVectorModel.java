...
public class WordVectorModel extends AbstractVectorModel<String>
{
    ...
    private static TreeMap<String, com.ahaxt.hanlp.mining.word2vec.Vector> loadVectorMap(String modelFileName) throws IOException
    {
        VectorsReader reader = new VectorsReader(modelFileName);
        reader.readVectorFile();
        TreeMap<String, com.ahaxt.hanlp.mining.word2vec.Vector> map = new TreeMap<String, com.ahaxt.hanlp.mining.word2vec.Vector>();
        for (int i = 0; i < reader.vocab.length; i++)
        {
            map.put(reader.vocab[i], new com.ahaxt.hanlp.mining.word2vec.Vector(reader.matrix[i]));
        }
        return map;
    }
    ...
    {
        com.ahaxt.hanlp.mining.word2vec.Vector a = storage.get(A);
        com.ahaxt.hanlp.mining.word2vec.Vector b = storage.get(B);
        com.ahaxt.hanlp.mining.word2vec.Vector c = storage.get(C);
